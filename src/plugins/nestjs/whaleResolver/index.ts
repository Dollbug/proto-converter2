...
export let enhanceProtoInfo: EnhancedProtoInfo
...
export const createImports: GenerateContent = ({ services }) => `
${NESTJS_DEPENDENCIES}
import { ${services
  .map(({ name }) => name)
...
export const assembleMethods = ({ methods }: protobuf.Service) =>
...
export const { createSource, buildFile: buildWhaleResolver } = new NestjsFile({
  fileType: 'resolver',
  createImports,
  createContent,
}).getBuildTools()

export default buildWhaleResolver
