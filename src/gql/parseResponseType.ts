...
function whaleParseResponseType(responseType: string) {
  const typeValue = lookupType(responseType) as protobuf.Type
  const result = Object.values(typeValue.fields)
    .map((f) => {
      parents = [responseType]
...
export function whaleParseResponse(responseType: string, info: ProtoInfo) {
  protoInfo = info
  return whaleParseResponseType(responseType)
}
